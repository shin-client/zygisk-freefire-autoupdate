...
struct Vector4 {
   ...
   */
  static inline void OrthoNormalize(Vector4 &normal, Vector4 &tangent, Vector4 &binormal);
   ...
   * @return: A new vector.
   */
  static inline Vector4 RotateTowards(Vector4 current, Vector4 target, float maxRadiansDelta, float maxMagnitudeDelta);
   ...
   * @param phi: The angle from the positive.Z .Xis to the vector.
   */
  static inline void ToSpherical(Vector4 vector, float &rad, float &theta, float &phi);
...
float Vector4::Component(Vector4 a, Vector4 b) { return Dot(a, b) / Magnitude(b); }
...
float Vector4::Distance(Vector4 a, Vector4 b) { return Vector4::Magnitude(a - b); }

float Vector4::Dot(Vector4 lhs, Vector4 rhs) { return lhs.X * rhs.X + lhs.Y * rhs.Y + lhs.Z * rhs.Z; }
...
Vector4 Vector4::LerpUnclamped(Vector4 a, Vector4 b, float t) { return (b - a) * t + a; }

float Vector4::Magnitude(Vector4 v) { return sqrt(SqrMagnitude(v)); }
...
Vector4 Vector4::Orthogonal(Vector4 v) { return v.Z < v.X ? Vector4(v.Y, -v.X, 0) : Vector4(0, -v.Z, v.Y); }

void Vector4::OrthoNormalize(Vector4 &normal, Vector4 &tangent, Vector4 &binormal) {
...
Vector4 Vector4::ProjectOnPlane(Vector4 vector, Vector4 planeNormal) { return Reject(vector, planeNormal); }

Vector4 Vector4::Reflect(Vector4 vector, Vector4 planeNormal) { return vector - 2 * Project(vector, planeNormal); }

Vector4 Vector4::Reject(Vector4 a, Vector4 b) { return a - Project(a, b); }

Vector4 Vector4::RotateTowards(Vector4 current, Vector4 target, float maxRadiansDelta, float maxMagnitudeDelta) {
  ...
  Vector4 newVector = current * cos(maxRadiansDelta) + Cross(axis, current) * sin(maxRadiansDelta);
  return newVector * newMag;
}

Vector4 Vector4::Scale(Vector4 a, Vector4 b) { return Vector4(a.X * b.X, a.Y * b.Y, a.Z * b.Z); }
...
float Vector4::SqrMagnitude(Vector4 v) { return v.X * v.X + v.Y * v.Y + v.Z * v.Z; }

void Vector4::ToSpherical(Vector4 vector, float &rad, float &theta, float &phi) {
...
bool operator==(const Vector4 lhs, const Vector4 rhs) { return lhs.X == rhs.X && lhs.Y == rhs.Y && lhs.Z == rhs.Z; }
